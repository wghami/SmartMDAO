...
**Your projects need no server of their own.** Since 1.26.0 each file is handled in its own
project's environment: the nearest folder with a `pyproject.toml` and a `.venv`, so the model's
libraries and its pinned SmartMDAO are the ones used. Every response says which environment was
used in `interpreter`. Pass `python=` or `project=` to choose. A project pinning SmartMDAO older
than 1.26.0 is handled in the server's environment, with a note saying so, until it upgrades.

Your own model is never cached either: the pipeline file *and the modules it imports from your
project* are read again on every call, so an edit shows up in the next one. (Before 1.24.0 an
edited sibling module kept its first version until the server restarted.)
...
|---|---|
| `pytest` reports fewer than 805 tests, with skips | `uv sync` did not install the extras — check for `openturns`, `mcp` and `clingo` |
